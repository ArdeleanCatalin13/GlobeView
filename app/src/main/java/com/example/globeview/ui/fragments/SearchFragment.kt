...
import com.example.globeview.util.Constants.Companion.SEARCH_COUNTRIES_TIME_DELAY
...
class SearchFragment : Fragment(R.layout.fragment_search) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        countriesAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(R.id.action_searchFragment2_to_countryFragment2, bundle)
        }
        ...
        binding.searchEdit.addTextChangedListener() { editable ->
            ...
            job = MainScope().launch {
                delay(SEARCH_COUNTRIES_TIME_DELAY)
                editable?.let {
                    if (editable.toString().isNotEmpty()) {
                        countriesViewModel.searchCountries(editable.toString())
                    }
                }
            }
        }

        countriesViewModel.searchCountries.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success<*> -> { hideProgressBar()
                    hideErrorMessage()
                    response.data?.let { countriesResponse ->
                        countriesAdapter.differ.submitList(arrayListOf(countriesResponse.data))
//                        val totalPages = countriesResponse.meta.total / Constants.QUERY_PAGE_SIZE + 2
//                        isLastPage = countriesViewModel.searchCountriesPage == totalPages
//                        if (isLastPage) {
...
//                        }
        ...
        retryButton.setOnClickListener {
            if (binding.searchEdit.text.toString().isNotEmpty()) {
                countriesViewModel.searchCountries(binding.searchEdit.text.toString())
            } else {
                hideErrorMessage()
            }
        }
